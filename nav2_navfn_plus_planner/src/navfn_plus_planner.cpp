  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  declare_parameter_if_not_declared(node_, name + ".tolerance_y", rclcpp::ParameterValue(0.12));
  node_->get_parameter(name + ".tolerance_y", tolerance_y_);
  ...
  const geometry_msgs::msg::PoseStamped & start, const geometry_msgs::msg::PoseStamped & goal)
{
  ...
  if (!makePlan(start.pose, goal.pose, tolerance_x_, tolerance_y_, path)) {
    RCLCPP_WARN(
      node_->get_logger(),
...
bool NavfnPlanner::makePlan(
  ...
  const double tolerance_x, const double tolerance_y, nav_msgs::msg::Path & plan)
{
...
bool NavfnPlanner::searchPoiArea(
  ...
  double tolerance_x, double tolerance_y)
{
  ...
  Eigen::Vector3d point = center_point;
  Eigen::Vector3d goal_poi{goal.position.x, goal.position.y, 0.0};

  Eigen::Quaterniond q;
  q.x() = goal.orientation.x;
  q.y() = goal.orientation.y;
  q.z() = goal.orientation.z;
  q.w() = goal.orientation.w;
  ...
  while (point.x() <= (center_point.x() + tolerance_x)) {
    ...
    while (point.y() <= (center_point.y() + tolerance_y)) {
      ...
      if (potential < POT_HIGH && sdist < best_sdist) {
        best_sdist = sdist;
        best_pose = map_frame_point;
        best_pose.orientation = goal.orientation;
        found_legal = true;
      }
      point.y() += resolution;
    }
...
NavfnPlanner::transformPointToMapcurrent(
  const Eigen::Vector3d & point,
  const Eigen::Vector3d & goal_poi,
  const Eigen::Matrix3d & rotation_matrix)
{
  Eigen::Vector3d point_map = rotation_matrix * point;
  point_map = point_map + goal_poi;

  geometry_msgs::msg::Pose pose;

  pose.position.x = point_map.x();
  pose.position.y = point_map.y();
